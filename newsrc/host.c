...
#else
cvar_t developer = {"developer", "1"};  // "0"};
#endif
...
void SV_DropClient(qboolean crash) {
	...
	if (!crash) {
		...
		if (host_client->edict && host_client->spawned) {
			// call the prog function for removing a client this will set the body to
			// a dead frame, among other things
...
void _Host_Frame(double elapsed_time) {
	...
	if (host_speeds.value) {
		...
		Con_Printf(
				"%3i tot %3i server %3i gfx %3i snd\n",
				pass1 + pass2 + pass3,
				pass1,
				pass2,
				pass3);
...
void Host_Frame(double elapsed_time) {
	double time1;
	double time2;
