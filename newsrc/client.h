...
typedef struct {
	...
	qboolean timedemo;
	int forcetrack;  // -1 = use normal cd track (CD audio removed)
	FILE *demofile;
...
// server signon
typedef struct {
	// since connecting to this server throw out the first couple, so the player
	...
	// clients view of time, should be between servertime and oldservertime to
	// generate a lerp point for other data
	double time;
